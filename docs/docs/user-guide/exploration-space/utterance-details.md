...
* **Utterance**: The utterance and its per-token [saliency](../../key-concepts/saliency.md) (if
  ...
  value.
  * If pre-processing steps are returned by the pipeline, blue arrows will be shown, allowing to display the utterance after each pre-processing step, from the raw utterance to the processed version sent to the model.
...
* **Predictions**:
    ...
    * By clicking on the blue arrows, the prediction results after each post-processing step can be viewed, including the model output before any post-processing.
